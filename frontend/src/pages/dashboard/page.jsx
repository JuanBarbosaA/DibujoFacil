        ...
        const fetchData = async () => {
            try {
                const [tutorialsRes, categoriesRes] = await Promise.all([
                    fetch(`http://localhost:5054/api/Tutorials?search=${searchTerm}&categoryId=${selectedCategory}&difficulty=${selectedDifficulty}`),
                    fetch('http://localhost:5054/api/Tutorials/categories')
                ]);

                if (!tutorialsRes.ok || !categoriesRes.ok) throw new Error('Error fetching data');
    ...
    const renderMediaContent = (content) => {
        return (
            <div key={content.id} className="space-y-2">
                <div className="px-2">
                    {content.title && <h3 className="font-semibold text-gray-800 text-sm mb-1">{content.title}</h3>}
                    {content.description && <p className="text-gray-600 text-xs italic">{content.description}</p>}
                </div>
                    ...
                    <article key={tutorial.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <Link to={`/tutorial/${tutorial.id}`} className="block hover:no-underline">
                            ...
                            <div className="p-4 space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        <span className="ml-1 text-gray-600">{tutorial.averageRating.toFixed(1)}</span>
                                    </div>
                                </div>

                                <div className="text-sm text-gray-500">⌛ {tutorial.estimatedDuration} minutos</div>

                                <div className="flex flex-wrap gap-2">
                                    {tutorial.categories.map(category => (
                                        <span key={category.id} className="px-2 py-1 bg-gray-100 text-gray-600 text-sm rounded-full">
                                            #{category.name}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="grid grid-cols-1 gap-4 p-4 bg-gray-50">
                                {tutorial.contents.sort((a, b) => a.order - b.order).map(renderMediaContent)}
                            </div>
